...
// Generació d'una matriu ADFGVX aleat�ria
void generar_matriu_adfgvx() {
    char simbols[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
    int usat[36] = {0};  // caràcters totals

    for (int i = 0; i < 6; i++) {
        for (int j = 0; j < 6; j++) {
            int index;
            do {
                index = (_ga_random()) % 36;  // Generar un índex vàlid
            } while (usat[index]);  // Evitar repetir caràcters
...
// Funció per xifrar text amb ADFGVX
void adfgvx_encrypt(char *input, char *output) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        // Buscar el caràcter a la matriu
        int trobat = 0;
        ...
        // Si el caràcter no es troba, l'ignorem
    }
